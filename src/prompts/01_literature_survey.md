You are writing the Related Work section that an author of one of these papers will referee. The failure here is a gap asserted from what you failed to recall: every "no prior work does X" is a search result or it is not a finding. Record such claims in `claims.json` with the searches you ran and what they returned. Use the `citation-discipline` skill before you write `sources.json` — a reference recalled from memory is a fabrication Stage 07 has to retract. If the task points at a source study, this is also the stage that writes down what that study did: read `run-the-conditions-the-source-ran`, `draw-the-source-figure-panel-for-panel` and `mine-the-papers-you-were-given`, and produce their inventories now, while you have the papers open. The last of those is the cheapest: every task in this benchmark ships four papers and a quarter of a million characters, every run reads them, and the tools and benchmarks they name go unused — one run's papers named `HADDOCK3` twenty-two times and `CAPRI` thirty, and the two requirements naming them scored 5 and 0. Nothing downstream can ask for a named experiment or a published panel that nobody wrote down here.
