...
def build_fire_goal(
    ...
    *,
    ...
    if model_catalog:
        ...
        maas_models = ", ".join(f"`{name}`" for name in model_catalog.get("vertex-maas", [])) or "none"
        model_block = (
            ...
            f"- Open-weight, served (`provider=\"vertex-maas\"`): {maas_models}\n\n"
            "The third line is the one to read twice if your task is about a *capability* "
            "contrast. Every model on the first two lines is frontier-grade, and several of "
            "these papers report effects that only appear below the frontier — prompt tricks "
            "that stop mattering once the model is strong enough, failure modes a good model "
            "does not have. Substituting a frontier model for the paper's weak arm does not "
            "shrink the effect, it *deletes the arm*, and the null you then report is a "
            "property of your model list rather than of the phenomenon. The open-weight line "
            "is the weak arm. Use it when the axis needs one.\n\n"
            "One operational note about it: the whole campaign shares one per-minute "
            "quota, so `vertex-maas` answers a burst instantly but returns HTTP 429 under "
            "a sustained wide batch. That is a queue, not an outage and not a missing "
            "model. Keep `max_workers` around 4-8 for it and the calls land.\n\n"
