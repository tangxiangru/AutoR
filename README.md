...
124 skills ship today: 68 general ones and 56 field-specific ones. Seventy-five of them were written against a scored arm's per-criterion losses on the
...
2. **Shape.** A skill may carry an `applies_when` regex, matched against this run's own research
   ...
   ResearchClawBench task statements they select 7, 5, 3, 2, 1, 1, 1 and 1 tasks each, twenty-six
   receive none of them, and no task receives more than four. `tools/skill_selectivity.py` prints the selection set
