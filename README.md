...
twenty-five ResearchClawBench tasks that lost, three per task, and three more against a second
scoring of one of those tasks under a single judge. **A run is not offered all of
...
2. **Shape.** A skill may carry an `applies_when` regex, matched against this run's own research
   ...
   ResearchClawBench briefs they select between 1 and 7 tasks each, twenty-six tasks receive none of
