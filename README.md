...
125 skills ship today: 66 general ones and 59 field-specific ones. Seventy-five of them were written against a scored arm's per-criterion losses on the
...
2. **Shape.** A skill may carry an `applies_when` regex, matched against this run's own research
   brief and data manifest. Eight skills are scoped this way today; measured over the forty
   ResearchClawBench briefs they select between one and seven tasks each, twenty-six tasks receive
...
3. **Pin.** There is one exception, and it is deliberate.
   ...
   tasks are pinned today, 246 pins between them, three per task at the fewest and fifteen at the
   most; 25 of the 246 are skills the two filters would have withheld — twenty field skills whose
   content applies outside their own field, and five unprefixed ones whose `applies_when` does not
   fire on that brief. **A run that matches an entry writes `skill_pins` into its `run_config.json` and a
