...
functions are parsed out of ``src/*.py``, the guards come from ``stage_graph.GUARDS`` and
the assessors from ``scorecard.FEATURES``, so a new gate fails this file rather than
joining the tree unclassified. Each declared path is resolved against ``build_run_paths``,
so a renamed ``RunPaths`` field fails too, and *whether a path is under* ``workspace/`` is
...
GATES: tuple[Gate, ...] = (
    ...
    Gate(
        ...
        ("repo:src/skills",),
        HARNESS,
        "Reads the AutoR checkout's skill pack, not the run tree. The copy installed into "
        "a run at .claude/skills/ is rebuilt from it on every resume.",
    ),
    Gate(
...
def _validators_in(source: str, module: str) -> set[str]:
    """Module-level ``validate_*`` functions, by AST rather than by grep.
