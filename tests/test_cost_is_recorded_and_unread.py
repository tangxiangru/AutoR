...
LITERAL_EXEMPTIONS: dict[str, str] = {
    "src/bench_call.py": (
        "Counts a foreign API's `output_tokens` out of a backend transcript. That is not a "
        "result event in the sense `is_result_event` matches and it never becomes a "
        "CallCost: the number is written into a benchmark's own metadata so a reader can "
        "see what an answer cost to produce, and nothing branches on it. Reading "
        "`TOKEN_FIELDS` here would tie AutoR's billing vocabulary to an external schema on "
        "the strength of one name agreeing."
    ),
    "src/terminal_ui.py": (
...
DISPATCH_SITES: tuple[Dispatch, ...] = (
    ...
    Dispatch(
        "src/bench_call.py::invoke",
        False,
        "A benchmark front end's own operator loop, the sibling of `src/rcb.py`'s. "
...
COST_AWARE_MODULES: dict[str, str] = {
    "src/bench_call.py": (
        "counts a transcript's output tokens into a benchmark run's own metadata, and "
