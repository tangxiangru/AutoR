...
class ValidityReviewStampTests(unittest.TestCase):
    ...
    def test_rewriting_one_finding_in_place_is_refused_at_the_same_row_count(self) -> None:
        """The cheapest tamper of all: same id, same number of rows, different objection.

        The test above it keeps two findings and writes one back, so a check that only
        compared *how many* rows were on the page would kill it too. This one stamps one
        finding and writes one finding, so the row count agrees and only the records
        disagree — which is the rule `validity_review_tamper` documents ("compares the
        finding records rather than the bytes") and the one that decides whether a
        critical confound can be re-filed as a missing caption unit.
        """
        self._record([FINDINGS[0]])
        self.review.write_text(
            json.dumps(
                {
                    "reviewed_stage": STAGE_05.slug,
        ...
        disagreement = validity_review_tamper(self.paths, STAGE_05.slug)
        self.assertTrue(disagreement, "an equal-length rewrite is the cheapest tamper")
        self.assertIn("holds 1 finding(s) where AutoR stamped 1", disagreement)

        # The rewrite does not become the population the gate counts, and answering the
        # softened row does not clear the refusal.
        self.assertEqual(
            [(item.severity, item.finding) for item in load_findings(self.paths, STAGE_05.slug)],
            [(FINDINGS[0].severity, FINDINGS[0].finding)],
        )
        self.response.write_text(_answers("V1"), encoding="utf-8")
        problems = validate_validity_response(self.paths, STAGE_06)
        self.assertEqual(len(problems), 1)
        self.assertIn("cannot be judged against the adversarial review", problems[0])

    def test_a_rewritten_finding_is_not_reported_as_a_dropped_one(self) -> None:
        """The sentence goes into `logs.txt`, so it has to name the edit that happened.

        `V1` is still on the page; what changed is what it says. Reporting that as
        "dropping V1" sends whoever reads the run log looking for a row that is right
        there, and the two edits need different responses from them.
        """
        self._record([FINDINGS[0]])
        self.review.write_text(
            json.dumps(
                {
                    "reviewed_stage": STAGE_05.slug,
                    "findings": [{"id": "V1", "severity": "minor", "finding": "A caption."}],
                }
            ),
            encoding="utf-8",
        )
        disagreement = validity_review_tamper(self.paths, STAGE_05.slug)
        self.assertIn("rewriting V1", disagreement)
        self.assertNotIn("dropping", disagreement)

    def test_a_partial_deletion_names_the_gone_row_and_the_rewritten_one_apart(self) -> None:
        """Both edits at once, which is what `test_swapping_in_a_softer_finding` does."""
        self._record()
        self.review.write_text(
            json.dumps(
                {
        ...
        disagreement = validity_review_tamper(self.paths, STAGE_05.slug)
        self.assertIn("dropping V2", disagreement)
        self.assertIn("rewriting V1", disagreement)
    ...
    def test_erasing_a_clean_review_is_refused_although_it_owed_nothing(self) -> None:
        """`findings: []` is a record too, and the one #202 spent a whole PR distinguishing.

        A reviewer that attacked the stage and raised nothing, and a reviewer that never
        returned, are the same empty list downstream — which is why the completion is
        stamped. Erasing the clean review puts the run back in the state where nothing on
        disk says which of the two happened, so the gate refuses even though the stage
        owes no answers. `docs/stage-contract.md` states that exception on the row.
        """
        self._record([])
        ...
        problems = validate_validity_response(self.paths, STAGE_06)
        self.assertEqual(len(problems), 1)
        self.assertIn("records 0 finding(s)", problems[0])
