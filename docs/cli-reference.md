...
| `python fire_agent.py` | The unattended FIRE-Bench agent. ([source](../fire_agent.py)) |
...
table row; `rcb_agent.py`'s 37 are covered as the six that are its own plus the
31 it shares with `main.py`, every one of the 31 spelled out by name.
...
Runs **one** [FIRE-Bench](firebench.md) task, in one of two arms, under the wall
clock the benchmark harness enforces. Writes `conclusion.md`, a `_meta.json` the
exit code is computed from, and the `log.log` the evaluator actually scores.
Never reads stdin.

```
python fire_agent.py [--bench-root PATH] [--task TASK] [--split {verified,unverified}]
                     [--list-tasks] [--profile {direct,pipeline}]
                     [--workspace PATH] [--log-file PATH] [--agent-id ID]
                     [--model MODEL] [--review-model MODEL]
                     [--operator {claude,codex}] [--review-operator {claude,codex}]
                     [--codex-command BIN] [--codex-sandbox MODE]
                     [--deadline-seconds N] [--reserve-seconds N]
                     [--first-stage STAGE] [--final-stage STAGE]
                     [--max-attempts N] [--max-operator-calls-per-stage N]
                     [--max-auto-skips N]
                     [--web-search {auto,gemini,native,off}]
                     [--cross-review {auto,gemini,off}] [--cross-review-model MODEL]
                     [--output-format FORMAT] [--attempt-index N]
                     [--print-goal] [--fake-operator]
```

### The task, and where the run happens

| Flag | Default | Description |
| --- | --- | --- |
| `--bench-root PATH` | `$FIREBENCH_ROOT`, then the current directory | A FIRE-Bench checkout. Refused unless `run_agent.py` sits beside `benchmark/papers/`: a directory that merely contains the tasks is a copy of them, not something the harness can run. |
| `--task TASK` | `$TASK_ID`, which is what the harness sets | Task id, for example `cot_in_planning`. |
| `--split {verified,unverified}` | search `verified` first | 35 human-curated tasks against 153 generated by the paper's own pipeline with no human review. |
| `--list-tasks` | off | Print the ids in a split and exit. |
| `--workspace PATH` | `$FIREBENCH_RUNS_DIR/<task>__<profile>__<stamp>`, and `$FIREBENCH_RUNS_DIR` defaults to `~/fire-bench-runs` | The sandbox. **Outside the checkout on purpose**: every shipped agent puts its sandbox in `<checkout>/runs/` and runs with `--dangerously-skip-permissions`, from where `../../benchmark/papers/<task>/conclusion.txt` — the answer the run is scored against — is two directories away and readable. |
| `--log-file PATH` | `<bench-root>/log/<agent-id>/<model>/<task>/<stamp>/log.log` | The file the evaluator reads. Its **last line** is the entire scored artifact. |
| `--agent-id ID` | `$AGENT_ID`, then `autor` | Appears in the log path and the log header; a trial uses it as the arm label. |

### The two arms

| Flag | Default | Description |
| --- | --- | --- |
| `--profile {direct,pipeline}` | `pipeline` | `direct` is one agentic operator call with the same goal, model, denied tools, sandbox and deadline — not one API call, since it can write and run code. `pipeline` is Stage 02 through Stage 05 plus one synthesis call. Pipeline minus direct is the pipeline's effect and nothing else. |
| `--first-stage STAGE` | `02_hypothesis_generation` | Above Stage 01 on purpose. Every task here is the rediscovery of a published finding whose paper is on the open web with its conclusion in the abstract, so a literature survey on this benchmark is a search for the answer key. |
| `--final-stage STAGE` | `05_experimentation` | Stage 06 is the first stage whose gate demands figures and the floor clamps to at least one; this benchmark scores no images, and four stages already fill the hour. |
| `--attempt-index N` | `0` | Which repeat of this (task, arm) this is. Recorded only. |

### The deadline

| Flag | Default | Description |
| --- | --- | --- |
| `--deadline-seconds N` | `3600` | The whole budget, matching the harness's own `TIME_LIMIT`. Per-stage timeouts are slices of what is left divided by the stages that have not yet written a summary, floored at 240 s. |
| `--reserve-seconds N` | `480` | Held back from the walk for the synthesis call and the publish. Only the publisher may spend it. |
| `--max-attempts N` | `2` | Attempts per stage. Small deliberately: AutoR's own default is unbounded, and an unbounded retry inside a hard deadline spends the whole budget on one stage and leaves no conclusion. |
| `--max-auto-skips N` | `1` | |
| `--max-operator-calls-per-stage N` | `4` | |

### Backend, reviewer and tools

| Flag | Default | Description |
| --- | --- | --- |
| `--model MODEL` | `$LLM_MODEL`, then `opus` | `opus` rather than AutoR's usual `sonnet`: a benchmark front end whose default is quota-exhausted is a front end nobody can run. Probe before trusting either. |
| `--review-model MODEL` | `--model` | Follows the executor by default, so one flag cannot leave the reviewer on a different pool. |
| `--operator`, `--review-operator` | `claude` | |
| `--web-search MODE` | `off` | See `--first-stage`. The denial covers `WebSearch` and `WebFetch` and is recorded per seat in `_meta.json`; `curl` lives inside `Bash`, so whether a run browsed is a question for its tool calls. |
| `--cross-review MODE` | `off` | A second model family in the loop is a second thing changing between the arms. |

### Diagnostics

| Flag | Default | Description |
| --- | --- | --- |
| `--print-goal` | off | Print the contract the agent would be given and exit, creating nothing. It renders the same data listing a real run would stage, so the printed document is the one the agent gets. |
| `--fake-operator` | off | Exercise the plumbing without calling a backend. Never produces a scoreable conclusion, by construction. |

### Exit code

| Code | Meaning |
| --- | --- |
| `0` | All six `FIRE_EXIT_CLAUSES` hold: a conclusion exists, is inside the length band, is not this adapter's fallback, reached the log file the evaluator reads, carries no content refusal, and the procedure completed. `--print-goal` also exits `0`. |
| `1` | Any clause failed, or the adapter raised. The failing names are in `_meta.json` under `exit_clause_failures`, and the code is a pure function of that same dictionary. |

---

## `tools/score_fire_run.py`

Scores one FIRE-Bench log with the benchmark's own evaluator, more than once,
and reports the spread beside the score.

```
python tools/score_fire_run.py --bench-root PATH --log-file PATH --task TASK
                               [--draws N] [--out-dir PATH] [--python PATH]
                               [--timeout SECONDS]
```

| Flag | Default | Description |
| --- | --- | --- |
| `--draws N` | `3` | How many times to score the same log. The measured range on one identical log was **43 F1 points**, so a single draw is judge noise rather than a measurement. A draw that produced no number — no conclusion in the log, an unknown task id, a judge failure — is counted and named, never averaged in as a zero. |
| `--python PATH` | `~/.venvs/firebench/bin/python` | The scoring interpreter. The evaluator needs `ragchecker`, `refchecker`, `litellm` and `torch`, which do not belong in the interpreter AutoR runs under. |
| `--out-dir PATH` | `<log dir>/_score/` | Per-draw JSON plus a `summary.json`. |
| `--timeout SECONDS` | `900` | Per draw. RefChecker retries a rejected model name forever with a ten-second sleep and no traceback, so a hung draw has to be killable without losing the finished ones. |

---

## `tools/fire_trial.py`

Runs a FIRE-Bench arm matrix and scores it.

```
python tools/fire_trial.py plan   --out PATH --tasks TASK ... [--arms ARM ...]
                                  [--bench-root PATH] [--runs-root PATH]
                                  [--model MODEL] [--deadline-seconds N] [--repeats N]
python tools/fire_trial.py run    --plan PATH [--concurrency N] [--force]
python tools/fire_trial.py score  --plan PATH [--draws N] [--force]
python tools/fire_trial.py report --plan PATH [--out PATH]
```

Three arms, because two cannot attribute a difference: `autor-pipeline`,
`autor-direct` (the same goal contract, one call) and `claude-stock` (the
benchmark's own agent on the raw `instruction.txt`). Pipeline minus direct is
the pipeline; direct minus stock is the goal contract. The report prints paired
per-task differences rather than a difference of means, and runs that produced
no conclusion get their own column instead of being averaged in as zeros.
