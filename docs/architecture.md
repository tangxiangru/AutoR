...
| [`fs_agent.py`](../fs_agent.py) | The FrontierScience-Research adapter entry point: one question per run under `--profile direct` (one operator call) or `--profile ideate` (AutoR entered at Stage 02 and stopped there), its own unattended defaults, `--web-search off` by default, and an exit code computed from the metadata it writes through `fs_exit_code`. |
...
| [`tools/score_fs_run.py`](../tools/score_fs_run.py) | Scores one FrontierScience answer against its own rubric with the paper's verbatim judge prompt, over `urllib.request` and no third-party client. Writes nothing when any draw is refused. |
| [`tools/fs_trial.py`](../tools/fs_trial.py) | Runs a paired FrontierScience trial — several answers at a time, one judge call at a time, resumable — and rebuilds its report from the state directory alone. |
...
| [`src/validity_review.py`](../src/validity_review.py) | The adversarial pass after Stages 05 and 06 (`REVIEWED_STAGE_NUMBERS`). Asks why the result is wrong across ten named failure modes rather than whether the stage is complete, and requires the next stage to answer every finding. Has no authority to approve or reject. `_write_review` stamps what it raised to `validity_review_stamp.json` at the run root as well as into `workspace/reviews/`, and `load_findings` reads the stamp, so the population the gate counts is not one the answering stage can edit. |
...
| [`src/trials.py`](../src/trials.py) | Paired A/B trials over archived runs tagged with `--trial`/`--capability`/`--arm`. `collect_pairs` excludes fake-provenance and stale-rubric arms; `sign_flip_p` is a two-sided paired permutation test, enumerated exactly up to `MAX_EXACT_PAIRS` pairs and sampled with `SIGN_FLIP_SEED` above it; `sign_flip_estimator` is the single branch both it and the report read, so the p-line names the computation that ran rather than the one the pair count predicts, and `attainable_p_floor` prints that computation's floor; `MIN_PAIRS_FOR_SIGNIFICANCE` labels anything smaller `underpowered`. Reachable from `main.py --trial-report` and from both benchmark trial drivers, and **no trial has been run**. |
| [`src/fs_trial.py`](../src/fs_trial.py) | The FrontierScience paired trial, where an arm is an *answer producer* rather than a git revision: `FsArmSpec` (`autor` or `direct`, with `forced_skills` deciding whether the child is launched with `--no-forced-skills`), `FsRunEnvironment`'s ten observed comparability fields folded into the pair key, ten `FS_ADMISSION_CLAUSES` each refusing a **pair**, `fold_duplicate_rows` for the byte-identical rows, `FS_MAX_REFUSAL_RATE` withholding the difference above a lopsided refusal rate, `next_actions` as the whole recovery policy in one pure function, and `format_fs_trial_report`. Fills `stage_fitness` with one key and `criterion_fitness` with nothing, and constructs no `Archive`. |
| [`src/trial_driver.py`](../src/trial_driver.py) | The half of a paired-trial driver that names no benchmark, shared by both: an `os.link` lock whose liveness test reads the pid, its `cmdline` and the boot id, `claim_stale_lock`, the `/proc` census (`is_backed_run`, `foreign_runs`, `autor_pids`, `AGENT_SCRIPT_NAMES`), atomic state writes for shared NFS, a stall watchdog that kills a process group, and three `git` readers. `marker` is a required keyword: a driver that let it default would read its own live lock as stale. |
...
| [`src/frontierscience.py`](../src/frontierscience.py) | The FrontierScience contract, and no socket or subprocess anywhere in it: the pinned dataset digests and `load_dataset`'s six assertions, `FsRow` addressed by row index because two rows share a `task_group_id`, the strict `parse_rubric`, `resolve_task_keys`, the three prompt blocks and `build_fs_goal` (fenced task first, always), `ensure_fs_workspace`, `export_answer`'s four ranked sources, `DirectAnswerWriter` and `AnswerSynthesizer` (which refuses when nothing was approved), `read_transcript_witness`, and the six `FS_EXIT_CLAUSES` the exit code is computed from. |
| [`src/fs_scoring.py`](../src/fs_scoring.py) | The pure half of the FrontierScience scorer: `FS_JUDGE_PROMPT` verbatim from the paper and pinned to a fixture, `response_text` filtering the Responses payload down to visible message text, `FS_VERDICT_PATTERN` taking the last anchored verdict, `judge_draw_failures`' five clauses, `aggregate_draws`, `refusal_reasons`, `format_spread` (never 0.00 from one draw) and `build_result`. A failed draw's points are `None`, never zero, because a real `VERDICT: 0` is a routine outcome here. |
