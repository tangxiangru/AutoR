...
| [`src/bench_call.py`](../src/bench_call.py) | The seam a benchmark front end makes **one** operator call through, and what it reads back: `_OperatorCall` over the same private `_prepare_invocation` / `_run_streaming_command` pair `ReportSynthesizer` uses, so the MCP config and the denied-tool list get no third place to drift apart; `read_transcript_witness` and its `TRANSCRIPT_FIELDS`, where `None` is "not observed" and never zero, so an unwitnessed run cannot satisfy a `browsing_tool_calls == 0` admission clause; `has_refusal`, `stage_answer_bodies`, `stages_approved_in`. Written for one benchmark adapter, now imported by `src/firebench.py` and `fire_agent.py`. |
