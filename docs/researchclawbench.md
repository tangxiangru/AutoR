...
## Where AutoR landed, before the control existed

Removed 2026-08-21. The four-agent table that stood here was scored at `MAX_IMAGES = 5`
where upstream sends fifteen, against criteria that are 60.6% images and arms that do not
ship the same number of figures — so its ordering was a property of the instrument, and
three copies of it were outliving the thing they measured. One copy is kept, in
[the lab notebook](researchclawbench-arms.md#before-the-control-existed), because the
corrections there are corrections *of it*. The current answer is
[the arms table](../README.md#four-later-arms-have-landed-and-they-are-ahead).

The workspaces all survive and none has been re-judged, so the ordering is unsettled rather
than reversed: AutoR's forty at
`/rmeng_data/robtang/autor-rcb-evidence/rcb_probe_workspaces`, the other 120 in the
benchmark's static export. Roughly 480 criterion judgments would settle it.
